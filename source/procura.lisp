...
(defun recalcular-profundidade (lista-sucessores lista-nos)
  "Função que recalcula a profundidade dos nós que se encontram em abertos ou fechados"
  (cond ((null lista-nos) nil)
        ((null lista-sucessores) lista-nos)
        (t (let ((no-atual (car lista-nos)))
            (recalcular-profundidade-auxiliar no-atual lista-sucessores)
            (recalcular-profundidade lista-sucessores (cdr lista-nos))
           )
        )
  )
)
...
(defun recalcular-profundidade-auxiliar (no-atual lista-sucessores &optional no-recalculado)
  (cond ((null lista-sucessores) no-recalculado)
        ((equal (no-estado no-atual) (no-estado (car lista-sucessores))) )
  )
)

(defun procurar-e-substituir-no-pai (no no-procurar no-substituto)
  (cond ((null (no-pai no)) nil)
        (t (cond ((equal (no-estado no) (no-estado no-procurar))
                 ...
                 (t (let ((pai (procurar-e-substituir-no-pai (no-pai no) no-procurar no-substituto)))
                      (cria-no (no-estado no) (1+ (no-profundidade pai)) (no-heuristica no) pai (no-pontuacao no)) 
                    )
                 )
           )
        )
...
(defun procurar-e-substituir-no-pai-teste ()
  (procurar-e-substituir-no-pai (cria-no (operador-2 (operador-2 (tabuleiro-jogado))) 5 0 (cria-no (operador-2 (tabuleiro-jogado)) 4 0 (cria-no (tabuleiro-jogado) 3 0 (cria-no (tabuleiro-teste)) 0) 0) 0) (cria-no (tabuleiro-jogado) 3 0 (cria-no (tabuleiro-teste)) 0) (cria-no (tabuleiro-jogado) 1 0 (cria-no (tabuleiro-teste)) 0))
)
...
(defun recalcular-profundidade-teste ()
  (recalcular-profundidade (list (cria-no (tabuleiro-jogado) 1 0 (cria-no (tabuleiro-teste)) 0) (cria-no (operador-2 (tabuleiro-jogado)) 1 0 (cria-no (tabuleiro-teste)) 0)) 
                           (list (cria-no (operador-2 (operador-2 (tabuleiro-jogado))) 5 0 (cria-no (operador-2 (tabuleiro-jogado)) 4 0 (cria-no (tabuleiro-jogado) 3 0 (cria-no (tabuleiro-teste)) 0) 0) 0) (cria-no (operador-2 (tabuleiro-jogado)) 4 0 (cria-no (tabuleiro-jogado) 3 0 (cria-no (tabuleiro-teste)) 0) 0) (cria-no (operador-3 (tabuleiro-jogado)) 4 0 (cria-no (tabuleiro-jogado) 3 0 (cria-no (tabuleiro-teste)) 0) 0) (cria-no (tabuleiro-jogado) 3 0 (cria-no (tabuleiro-teste)) 0) (cria-no (tabuleiro-teste) 0 0 nil 0)))
)
;;; Heurísticas
