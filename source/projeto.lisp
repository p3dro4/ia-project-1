...
(defun escolher-heuristica (problema)
  ...
  (format t "#~43,1,,:@<2 - Heuristica base melhorada~>#~%")
  (format t "#~43,1,,:@<0 - Voltar~>#~%")
  (format t "#~43,1,,:@<~>#~%")
  (format t "~45,1,,'#<~%~>")
  (let ((opcao (ler-opcao 2 "Heuristica > ")))
    (cond ((= opcao 0) (escolher-algoritmo problema))
          ...
          ((= opcao 2) 
            (let* ((resultado (executar-algoritmo-problema problema 'aestrela :funcao-heuristica (lambda (estado pontuacao) (funcall 'heuristica-base-melhorada estado (problema-objetivo problema) pontuacao))))
                   (experiencia (list problema (list (list "A* (Heuristica Base Melhorada)" resultado)))))
              (escreve-experiencia experiencia t)
              (escreve-experiencia experiencia)
              (executar-outro-algoritmo problema)
            )
